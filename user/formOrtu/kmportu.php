...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Insert data into t_responden_ortu table
    $query = $db->prepare("INSERT INTO t_responden_ortu (responden_ortu_id, survey_id, responden_tanggal, responden_nama, responden_jk, responden_umur, responden_hp, responden_pendidikan, responden_pekerjaan, responden_penghasilan, mahasiswa_nim, mahasiswa_nama, mahasiswa_prodi) VALUES (NULL, 9, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $query->bind_param('sssisssssss', $_SESSION["Tanggal"], $_SESSION["Nama"], $_SESSION["JK"], $_SESSION["Umur"], $_SESSION["HP"], $_SESSION["Pendidikan"], $_SESSION["Pekerjaan"], $_SESSION["Penghasilan"], $_SESSION["Mahasiswa_NIM"], $_SESSION["Mahasiswa_Nama"], $_SESSION["Mahasiswa_Prodi"]);
    ...
    // Insert data into t_jawaban_ortu table
    if (isset($_POST['jawaban']) && is_array($_POST['jawaban'])) {
        foreach ($_POST['jawaban'] as $soal_id => $jawaban) {
            $query = $db->prepare('INSERT INTO t_jawaban_ortu (jawaban_ortu_id, responden_ortu_id, soal_id, jawaban) VALUES (NULL, ?, ?, ?)');
            $query->bind_param('iis', $lastIdInserted, $soal_id, $jawaban);
            $query->execute();
        }
    }

    // Redirect to pkdosen.php
    header('Location: lkportu.php');
    exit();
}
?>
        ...
        <div class="row">
          <div class="col-12">
            <div class="card">
              ...
              <h2 style="text-align: center;">Kuesioner Orang Tua / Wali</h2>
              <h2 style="text-align: center;">Kemajuan Mahasiswa sebagai hasil Pengajaran di Politeknik Negeri Malang</h2>
              <br>
...
<div class="row">
  ...
  <table class="table table-sm table-bordered">
    ...
    $Survey_id = isset($_GET['Survey_id']) ? $_GET['Survey_id'] : 9;
