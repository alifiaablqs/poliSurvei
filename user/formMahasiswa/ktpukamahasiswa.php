...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Insert data into t_responden_mahasiswa table
    $query = $db->prepare("INSERT INTO t_responden_mahasiswa (responden_mahasiswa_id, Survey_id, responden_tanggal, responden_nama, responden_prodi, responden_nim, responden_email, responden_hp, tahun_masuk) VALUES (NULL, 2, ?, ?, ?, ?, ?, ?, ?)");
    $query->bind_param('sssssss', $_SESSION["Tanggal"], $_SESSION["Nama"], $_SESSION["Prodi"], $_SESSION["NIM"],$_SESSION["Email"], $_SESSION["HP"],  $_SESSION["Tahun_Masuk"]);
    ...
    // Insert data into t_jawaban_mahasiswa table
    if (isset($_POST['jawaban']) && is_array($_POST['jawaban'])) {
        foreach ($_POST['jawaban'] as $soal_id => $jawaban) {
            $query = $db->prepare('INSERT INTO t_jawaban_mahasiswa (jawaban_mahasiswa_id, responden_mahasiswa_id, soal_id, jawaban) VALUES (NULL, ?, ?, ?)');
            $query->bind_param('iis', $lastIdInserted, $soal_id, $jawaban);
            $query->execute();
        }
    }

    // Redirect to kpamahasiswa.php
    header('Location: lmanamahasiswa.php');
    exit();
}
?>
        ...
        <div class="row">
          <div class="col-12">
            <div class="card">
              ...
              <h2 style="text-align: center;">Kuesioner Mahasiswa</h2>
              <h2 style="text-align: center;">Kemampuan Tenaga Pengajar Untuk Kegiatan Akademik Politeknik Negeri Malang</h2>
              <br>
...
<div class="row">
  ...
  <table class="table table-sm table-bordered">
    ...
    $Survey_id = isset($_GET['Survey_id']) ? $_GET['Survey_id'] : 2;
